...
const GOOGLE_CLIENT_ID =
  '483421872463-fbpp0fe6s8r91h1i62vfubdls19auafg.apps.googleusercontent.com'
...
const foodCategories = [
  ...
  },
  {
    ...
    exclude: 'Do not select for peanuts or ordinary green peas.'
  },
  {
  ...
  },
  {
    key: 'dairy',
    label: 'Dairy',
    description: 'Milk, ice cream, cream, yogurt, and fresh or soft cheeses.',
    exclude:
  ...
  },
  {
    ...
    exclude: 'Do not select for decaf coffee or caffeine-free drinks.'
  },
  {
    key: 'alcohol',
    label: 'Alcohol',
    description: 'Beer, wine, liquor, cocktails, and other alcoholic drinks.',
    exclude: 'Do not select for nonalcoholic drinks.'
  },
  {
    key: 'carbonated',
    label: 'Carbonated',
    ...
    exclude: 'Do not select for still water or other non-carbonated drinks.'
  },
  {
    key: 'fermented_aged',
  ...
  },
  {
    ...
    exclude: 'Do not select for fresh, unprocessed meat, poultry, or fish.'
  },
  {
    key: 'leftovers',
    ...
    exclude: 'Do not select for food cooked and eaten immediately.'
  }
]
...
function selectSymptom (key) {
  ...
  document.getElementById(
    'detailHeading'
  ).textContent = `${symptom.label}: what kind?`
...
async function syncPendingEntries () {
  ...
  for (const entry of pending) {
    try {
      const row =
        entry.type === 'symptom' ? getSymptomRow(entry) : getFoodRow(entry)
  ...
  if (remaining.length === 0 && pending.length > 0) {
    document.getElementById('status').textContent =
      'All pending entries synced to Google Sheets.'
  }
}
...
async function saveSymptomEntry (note = '') {
  ...
  document.getElementById(
    'status'
  ).textContent = `Saved: ${symptomLabel} · ${entry.severity}/4`
...
async function saveFoodEntry (note = '') {
  ...
  resetFood()
  showScreen('home')
  document.getElementById('status').textContent = 'Food logged.'
...
document
  .getElementById('backToHomeFromSymptoms')
...
document.getElementById('syncButton').addEventListener('click', async () => {
  try {
    ...
    if (getPendingEntries().length > 0) {
      document.getElementById('status').textContent =
        'Some entries could not be synced. Check your Google Sheets connection.'
    }
  } catch (error) {
    document.getElementById('status').textContent = error.message
  }
